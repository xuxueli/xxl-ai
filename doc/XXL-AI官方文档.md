...
- **一套仓库，三种模式**：采用 Monorepo 统一托管三种工程形态 —— 单体（xxl-ai-admin）、前后端分离 Vue（xxl-ai-api + xxl-ai-ui-vue）、前后端分离 React（xxl-ai-api + xxl-ai-ui-react）。按业务诉求三选一，统一版本管理、一键构建、按需部署，切换成本极低。
...
- **AI + SKILL，开发再提速**：仓库内置 `xxl-ai-monolith / xxl-ai-vue / xxl-ai-react` 三大开发 SKILL，AI 编程助手（如 opencode）可自动识别并加载，按平台既定规范直生等价代码、自动落位并附校验清单，进一步拉高业务落地效率。
...
| [https://gitee.com/xuxueli0323/xxl-ai](https://gitee.com/xuxueli0323/xxl-ai) | [Download](https://gitee.com/xuxueli0323/xxl-ai/releases) |
...
于2024年11月，xxl-code-generator 项目更名 XXL-AI，整合xxl-permission、xxl-code-generator多个历史项目，并吸收 XXL-JOB、XXL-CONF 等系列开源软件所沉淀的中后台能力。
XXL-AI 定位为 快速开发平台，整合流行前后端技术能力，致力为中小企业与个人开发者打造开箱即用的快速开发解决方案。

于2026年7月，XXL-AI 整合主流技术栈，推出前后端分离版本。业务方可以根据实际业务诉求，自行选择 单体版本、前后端分离版本。
...
XXL-AI 提供三种运行模式，请先明确业务选型，再按对应方式部署：

| 运行模式 | 组成模块 | 端口 | 适用场景 |
|---|---|---|---|
| 单体模式 | xxl-ai-admin | 8080 | 中小后台、内网系统、快速交付，服务端渲染开箱即用 |
| 前后端分离 Vue 模式 | xxl-ai-api + xxl-ai-ui-vue | 8090 + 3000 | 前端使用 Vue3 + Element Plus，团队分工协作 |
| 前后端分离 React 模式 | xxl-ai-api + xxl-ai-ui-react | 8090 + 4000 | 前端使用 React + Ant Design，团队分工协作 |
...
```
/xxl-ai/doc/db/
    - tables_xxl_ai.sql                   ：公共初始化SQL脚本【必须】
    - tables_xxl_ai_monolith.sql          ：单体模式初始化SQL脚本【可选】
    - tables_xxl_ai_modular_vue.sql       ：前后端分离（Vue）模式初始化SQL脚本【可选】
    - tables_xxl_ai_modular_react.sql     ：前后端分离（React）模式初始化SQL脚本【可选】
```

补充说明：
- 首先初始化 “公共初始化SQL脚本”（`tables_xxl_ai.sql`）；
...
```
- xxl-ai/
    - xxl-ai-admin              ：【单体模式】单体服务
    - xxl-ai-api                ：【前后端分离】后端API服务
    - xxl-ai-ui/
        - xxl-ai-ui-vue         ：【前后端分离】前端UI服务（Vue3 版本）
        - xxl-ai-ui-react       ：【前后端分离】前端UI服务（React 版本）
```

编译方式：
- 后端模块：仓库根目录执行 `mvn clean package -Dmaven.test.skip=true`，一键编译全部 Maven 模块；
- 前端模块：进入 `xxl-ai-ui-vue` 或 `xxl-ai-ui-react` 目录执行 `npm install` 安装依赖。
...
```
/xxl-ai/xxl-ai-admin/src/main/resources/application.properties
```

配置内容说明：

```
### xxl-ai, datasource。 数据库配置，与 ”2.2 初始化数据库“ 章节初始化的数据库保持一致。
...
开发模式，进入 `xxl-ai-admin` 目录启动即可：

```
cd /xxl-ai/xxl-ai-admin
mvn spring-boot:run
```
...
配置内容说明（数据库配置，与 ”2.2 初始化数据库“ 章节初始化的数据库保持一致；具体是执行 `tables_xxl_ai_modular_vue.sql`）：
...
```
/xxl-ai/xxl-ai-ui/xxl-ai-ui-vue/.env.development   # 开发环境
/xxl-ai/xxl-ai-ui/xxl-ai-ui-vue/.env.staging       # 预发布环境
/xxl-ai/xxl-ai-ui/xxl-ai-ui-vue/.env.production    # 生产环境
```
...
```
cd /xxl-ai/xxl-ai-ui/xxl-ai-ui-vue
npm install
npm run dev
```
...
- 部署项目：xxl-ai-api + xxl-ai-ui-react
- 项目说明：前后端分离模式，后端 API 与前端 UI 独立部署、独立运行。后端选型 "SpringBoot/Mybatis/XXL-SSO"，前端选型 "React/Vite/AntDesign/TypeScript"。

#### 步骤一：启动后端服务

后端配置文件地址与配置方式同 “2.5 方式二” 步骤一，端口默认 `8090`；数据库初始化执行 `tables_xxl_ai_modular_react.sql`。
...
```
/xxl-ai/xxl-ai-ui/xxl-ai-ui-react/.env.development   # 开发环境
/xxl-ai/xxl-ai-ui/xxl-ai-ui-react/.env.staging       # 预发布环境
/xxl-ai/xxl-ai-ui/xxl-ai-ui-react/.env.production    # 生产环境
```
...
```
cd /xxl-ai/xxl-ai-ui/xxl-ai-ui-react
npm install
npm run dev
```
...
XXL-AI 三种运行模式（单体、前后端分离 Vue、前后端分离 React）能力一致，仅前端技术栈与界面形态不同。平台后台按“组织权限、系统管理、系统工具、AI 能力”四个板块组织：
...
> 配图说明：下文每项能力均提供 单体模式（xxl-ai-admin）、Vue 模式（xxl-ai-ui-vue）、React 模式（xxl-ai-ui-react）三种界面截图，可直观对比三种模式之下同一能力的操作界面；功能与操作逻辑三者完全一致。

### 3.2 登录与系统首页

三种运行模式均基于 XXL-SSO 提供安全登录，后端账户体系与权限体系完全一致，仅前端界面形态不同。登录页（从左至右依次为：单体模式、Vue 模式、React 模式）：

<p align="center"><b>Vue 模式</b> <img src="https://www.xuxueli.com/project/static/xxl-ai/images/vue/img_vue_login.png" width="300"/></p>

系统首页：

<p align="center"><b>Vue 模式</b> <img src="https://www.xuxueli.com/project/static/xxl-ai/images/vue/img_vue_index.png" width="300"/></p>
...
<p align="center"><b>Vue 模式</b> <img src="https://www.xuxueli.com/project/static/xxl-ai/images/vue/img_vue_user.png" width="300"/></p>

#### 3.3.2 角色管理

支持针对系统权限角色进行动态管理，进行角色新增、管理、菜单分配等操作。

<p align="center"><b>Vue 模式</b> <img src="https://www.xuxueli.com/project/static/xxl-ai/images/vue/img_vue_role.png" width="300"/></p>

#### 3.3.3 资源管理

支持针对系统资源进行细粒度管理，支持目录（type=0）、菜单（type=1）、按钮（type=2）等多类型资源管理，进行新增、管理等操作。

<p align="center"><b>Vue 模式</b> <img src="https://www.xuxueli.com/project/static/xxl-ai/images/vue/img_vue_resource.png" width="300"/></p>

#### 3.3.4 组织管理

支持针对部门组织架构进行管理，进行多层级组织架构的新增、管理、排序等操作。

<p align="center"><b>Vue 模式</b> <img src="https://www.xuxueli.com/project/static/xxl-ai/images/vue/img_vue_org.png" width="300"/></p>
...
<p align="center"><b>Vue 模式</b> <img src="https://www.xuxueli.com/project/static/xxl-ai/images/vue/img_vue_dict.png" width="300"/></p>

#### 3.4.2 配置中心

针对常用业务数据进行动态配置，包括配置定义、配置值管理等，在线管理并实时生效。

<p align="center"><b>Vue 模式</b> <img src="https://www.xuxueli.com/project/static/xxl-ai/images/vue/img_vue_conf.png" width="300"/></p>

#### 3.4.3 站内消息

针对系统用户推送站内消息，覆盖消息发布及维护管理、触达及已读查阅分析等完整闭环。

<p align="center"><b>Vue 模式</b> <img src="https://www.xuxueli.com/project/static/xxl-ai/images/vue/img_vue_msg.png" width="300"/></p>

#### 3.4.4 审计日志

记录系统操作及活动日志，用于系统的监控、审计和安全分析，可快速了解系统运行情况、发现异常行为、追溯问题源头。

<p align="center"><b>Vue 模式</b> <img src="https://www.xuxueli.com/project/static/xxl-ai/images/vue/img_vue_log.png" width="300"/></p>
...
<p align="center"><b>Vue 模式</b> <img src="https://www.xuxueli.com/project/static/xxl-ai/images/vue/img_vue_codegen.png" width="300"/></p>
...
<p align="center"><b>Vue 模式</b> <img src="https://www.xuxueli.com/project/static/xxl-ai/images/vue/img_vue_codegen2.png" width="300"/></p>

#### 第三步：预览与批量生成（前后端分离模式）

点击「预览」可在线逐文件查看全部生成代码；点击「批量生成」可下载 zip 产物（后端 6 件套 + 前端 vue3/react 三文件 + `-init.sql` 菜单权限 SQL）。

<p align="center"><b>Vue 模式</b> <img src="https://www.xuxueli.com/project/static/xxl-ai/images/vue/img_vue_codegen3.png" width="300"/></p>
...
<p align="center"><b>Vue 模式</b> <img src="https://www.xuxueli.com/project/static/xxl-ai/images/vue/img_vue_form.png" width="300"/></p>
...
<p align="center"><b>Vue 模式</b> <img src="https://www.xuxueli.com/project/static/xxl-ai/images/vue/img_vue_theme.png" width="300"/></p>

#### 3.7.2 个人中心

前后端分离模式提供个人中心页，支持个人信息维护、修改密码等操作：

<p align="center"><b>Vue 模式</b> <img src="https://www.xuxueli.com/project/static/xxl-ai/images/vue/img_vue_profile.png" width="300"/></p>

#### 3.7.3 帮助

三种模式均内置帮助页面，展示平台能力与快捷键等使用说明：

<p align="center"><b>Vue 模式</b> <img src="https://www.xuxueli.com/project/static/xxl-ai/images/vue/img_vue_help.png" width="300"/></p>


## 四、新增业务模块

新增一个带列表页、增删改查的业务模块，是 XXL-AI 上最典型的开发动作。平台提供三种方式，投入成本与产出形态各有侧重，可按团队情况选择：
...
> 三种方式共用同一套运行模式 Skill 规范（`xxl-ai-vue / xxl-ai-react / xxl-ai-monolith`），落位路径与代码规范完全一致，可自由切换混合使用。
...
.agents/
└── skills/
    ├── xxl-ai-vue        # 前后端分离 Vue 模式标准作业 Skill
    ├── xxl-ai-react      # 前后端分离 React 模式标准作业 Skill
    └── xxl-ai-monolith   # 单体模式标准作业 Skill
```

每个 SKILL 内置：工程结构说明、后端/前端落位清单、代码骨架模板、菜单权限 SQL 模板、校验清单，以及参考样例文件，保证 AI 产物与平台规范严格一致。

以 opencode + `xxl-ai-vue` 为例，完整开发一个「产品信息管理」业务模块（维护产品名称、介绍、生产时间等）的实操演示如下：

#### 第一步：描述需求

打开 AI 编程助手，将 XXL-AI 仓库作为工作目录打开，直接用 `/` 前缀调用对应 Skill，并描述业务诉求：

<p align="center"><img src="https://www.xuxueli.com/project/static/xxl-ai/images/skill/img_skill_1.png" width="900"/></p>

> 需求示例：“`/xxl-ai-vue` 新增一个功能模块，管理产品信息，维护 产品名称、介绍、生产时间 等。”

#### 第二步：需求澄清与确认

Skill 强制 AI 动手前先完成需求澄清，就模块/业务命名、核心字段、页面形态、菜单/按钮权限、出码方式、验证范围等连续提问并确认细节：

<p align="center"><img src="https://www.xuxueli.com/project/static/xxl-ai/images/skill/img_skill_2.png" width="900"/></p>

全部问题确认完毕后，AI 汇总需求细节，形成本需求的数据模型与开发方案：

<p align="center"><img src="https://www.xuxueli.com/project/static/xxl-ai/images/skill/img_skill_3.png" width="800"/></p>
...
<p align="center"><img src="https://www.xuxueli.com/project/static/xxl-ai/images/skill/img_skill_4.png" width="900"/></p>

#### 第四步：交付与验证

开发完成后，AI 交付 SQL、前端、后端全套产物与验证结果摘要，并补充特殊说明（如数据库中遗留 Skill 自动生成的测试数据等）：

<p align="center"><img src="https://www.xuxueli.com/project/static/xxl-ai/images/skill/img_skill_5.png" width="900"/></p>

#### 第五步：联调验收

AI 自动启动 api(8090) 与前端(3000) 完成本地联调后，人工在浏览器刷新登录，从菜单进入「产品管理」页面，即可开展数据查询与增删改操作（含搜索、分页、弹窗表单等）：

<p align="center"><img src="https://www.xuxueli.com/project/static/xxl-ai/images/skill/img_skill_6.png" width="900"/></p>

<p align="center"><img src="https://www.xuxueli.com/project/static/xxl-ai/images/skill/img_skill_7.png" width="900"/></p>

#### 第六步：代码合并

AI 生成的代码经 review 确认后，形成代码 PR 并提交到仓库：

<p align="center"><img src="https://www.xuxueli.com/project/static/xxl-ai/images/skill/img_skill_8.png" width="400"/></p>
...
- 进入「系统工具 - 代码生成」页面，点击新增，录入业务建表 SQL（遵循 `xxl_ai_` 前缀规范）；
...
按 `xxl_ai_` 前缀规范创建业务表，公共字段 `id / add_time / update_time`，状态字段使用 `TINYINT`，字段一律 `COMMENT` 注释：
...
在 `xxl-ai-api` 工程 `business` 扩展包中落位 6 件套：
...
在 `xxl-ai-ui-vue` 工程落位 3 文件，并在 `src/types/api.ts` barrel 中补一行导出：
...
启动 `xxl-ai-api` + `xxl-ai-ui-vue`（或选择其他运行模式），验证：菜单可见、列表 / 新增 / 修改 / 删除 / 搜索可用、按钮级权限生效（未授权用户对应按钮隐藏）、空参数后端友好提示。
...
- 后端统一通过 Maven 父工程管理，根目录 `pom.xml` 集中维护模块依赖版本（如 SpringBoot、Mybatis、MySQL、XXL-SSO 等），子模块 `xxl-ai-admin`、`xxl-ai-api` 继承使用；
- 前端模块 `xxl-ai-ui-vue`、`xxl-ai-ui-react` 独立通过 npm 管理依赖（Vue3/Vite/ElementPlus 或者 React/Vite/AntDesign），与后端 Maven 工程解耦；
...
│   ├── xxl-ai-monolith/                     # 单体模式开发 Skill
│   ├── xxl-ai-vue/                          # 前后端分离（Vue）开发 Skill
│   └── xxl-ai-react/                        # 前后端分离（React）开发 Skill
│
├── doc/                                       # 文档目录
│   ├── db/                                    # 数据库初始化SQL脚本目录
│   │   ├── tables_xxl_ai.sql                # 公共初始化SQL脚本【必须】
│   │   ├── tables_xxl_ai_monolith.sql       # 单体模式初始化SQL脚本【可选】
│   │   ├── tables_xxl_ai_modular_vue.sql    # 前后端分离（Vue）初始化SQL脚本【可选】
│   │   ├── tables_xxl_ai_modular_react.sql  # 前后端分离（React）初始化SQL脚本【可选】
│   │   └── plugin/                            # 扩展插件 SQL 脚本
│   ├── images/                                # 文档图片目录
│   └── XXL-AI官方文档.md                    # 官方文档
│
...
│           └── i18n/                          # 国际化资源文件
│
└── xxl-ai-ui/                               # 【前后端分离】前端UI模块目录
    ├── xxl-ai-ui-vue/                       # 前端UI服务（Vue3 + Vite + ElementPlus + TS）
    │   ├── package.json                       # 前端依赖配置
    ...
    │
    └── xxl-ai-ui-react/                     # 前端UI服务（React + Vite + AntDesign + TS）
        ├── package.json                       # 前端依赖配置
        ├── vite.config.ts                     # Vite构建配置
        └── src/
            ├── main.tsx                       # 入口文件
...
补充说明：
- 构建：后端模块在仓库根目录执行 `mvn clean package` 即可一键编译全部 Maven 模块；前端模块进入 `xxl-ai-ui-vue` 或 `xxl-ai-ui-react` 目录执行 `npm install`、`npm run build` 构建；
- 部署：单体模式部署 `xxl-ai-admin` 模块；前后端分离模式部署 `xxl-ai-api` + 前端Vue或React模块，参考 “2.4 / 2.5 / 2.6”；
- 扩展：新增业务模块时，可在各模块 `business` 扩展包中开发，并配套放置 Mapper 映射文件、模板文件及配置文件，参考 “四、新增业务模块”。

### 5.2、三种运行模式

XXL-AI 通过 Monorepo 一套仓库同时提供三种运行模式，统一后端核心能力（登录鉴权、RBAC 权限、系统管理、代码生成等），按技术选型与交付形态选择前端形态：
...
│  xxl-ai-admin │  xxl-ai-api + ui-vue       │ xxl-ai-api + ui-react
│                 │                              │                     │
...
xxl_ai_user           : 用户表
xxl_ai_role           : 角色表
xxl_ai_resource       : 资源表，菜单Page、功能Btn等
xxl_ai_user_role      : 用户-角色关系表
xxl_ai_role_res       : 角色-资源关系表
```

资源定义与授权链路：
- 资源分三级：目录（type=0）、菜单（type=1）、按钮（type=2）；菜单 `url` 同时充当前端路由 path 与页面组件定位 key，新增页面无需改动路由代码；
- 授权链路：菜单/按钮资源 → `xxl_ai_role_res` 授权给角色 → `xxl_ai_user_role` 将角色分配给用户；
...
```
.agents/skills/
├── xxl-ai-monolith/SKILL.md      # 单体模式：SpringBoot + FreeMarker + AdminLTE 标准作业
├── xxl-ai-vue/SKILL.md           # Vue 分离模式：xxl-ai-api + xxl-ai-ui-vue 标准作业
└── xxl-ai-react/SKILL.md         # React 分离模式：xxl-ai-api + xxl-ai-ui-react 标准作业
```
...
- 1、【整合】项目更名 XXL-AI，整合xxl-permission、xxl-code-generator多个历史项目；定位为 快速开发平台，整合流行前后端技术能力，致力为中小企业与个人开发者打造开箱即用的快速开发解决方案。
...
- 1、【安全】登录安全升级，密码加密处理算法从Md5改为Sha256；
    ...
    UPDATE xxl_ai_user t SET t.password = '8d969eef6ecad3c29a3a629280e686cf0c3f5d5a86aff3ca12020c923adc6c92' WHERE t.username = {用户名};
    ```
...
- 1、【新增】XXL-AI 前后端分离版本 发布：支持 单体项目、前后端分离项目 多模式；
  - 前后端分离模式：
    - 前端UI模块：xxl-ai-ui-vue 或 xxl-ai-ui-react，支持独立部署，提供前端UI服务；
    - 后端API模块：xxl-ai-api，支持独立部署，提供后端API服务；
  - 单体模式：xxl-ai-admin，提供前后端一体化服务；
...
- 3、【新增】AI + SKILL 驱动开发：仓库内置 `xxl-ai-monolith / xxl-ai-vue / xxl-ai-react` 三大开发 SKILL，AI 编程助手可自动识别并加载，按平台规范直生业务代码、自动落位并附校验清单，加速业务开发；
...
<details>
    ...
    ```
    // 1、进入仓库：使用 AI 编程助手（如 opencode）打开 XXL-AI 仓库，根据项目运行模式选择对应 SKILL（xxl-ai-monolith / xxl-ai-vue / xxl-ai-react）；
